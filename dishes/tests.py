...
class SearchDishTestCase(TestCase):
    ...
    def test_query_search_filter(self):
        search_result = Dish.objects.filter(
            name__icontains=self.search_phrase).values()
...
class IngredientFormTestCase(TestCase):
    ...
    def test_name_starting_lowercase(self):
        ...
        self.assertEqual(
            form.errors["name"], ["Should start with an uppercase letter!"]
        )
    ...
    def test_name_more_than_50(self):
        ...
        self.assertEqual(
            form.errors["name"],
            ["Ingredient name length should be less than 50 symbols!"],
        )
