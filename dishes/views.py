...
class SearchResultsView(ListView):
    ...
    def get_queryset(self):
        ...
        object_list = Dish.objects.filter(name__icontains=query_q)
        return object_list
    ...
    def get_queryset(self):
        ...
        if query_filter and query_filter == "asc":
            ...
            dishes = Dish.objects.filter(
                created_at__range=[query_from, query_to]
            )
        else:
            dishes = Dish.objects.all().order_by("-created_at")
        return dishes
...
class OrderIngredientCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        dish_id = self.request.GET.get("id")
        if self.request.POST:
            context["oi_formset"] = OrderIngredientFormset(self.request.POST)
        else:
            dish_name = Dish.objects.filter(pk=dish_id)
            user = self.request.user
            username = User.objects.filter(username=user)
            ingredients_queryset = DishIngredient.objects.filter(dish=dish_id)
            ...
            initial_data = [
                {"ingredient": i, "amount": j} for i, j in data.items()
            ]
